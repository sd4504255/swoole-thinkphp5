...
$cmd="/usr/local/php7/bin/php";//php的绝对路径
function syncServer()
{
//    echo (yield ['rpc']) ."\n";
//    echo (yield ['mysqlpool']) ."\n";
//    echo (yield ['vmstat']) ."\n";
    echo (yield ['websocket']) ."\n";
//    echo (yield ['task']) ."\n";
...
//                break;
            case 'swoolelive':
                ...
                break;
            case 'websocket':
                foreach(glob(__DIR__.'/swoole/WebSocketServer.php') as $start_file)
                {
                    exec($cmd.' '.$start_file);
                }
                echo "WebSocketServer SERVEICE START ...\n";//socket链接
                $gen->send('WebSocketServer SERVEICE SUCCESS!');
                asyncCaller($gen);
                break;
            case 'task':
...
if(!isset($ser_ser[1])){
    ...
    switch ($ser_ser[1]) {
        ...
        case 'stop':
            ...
            break;
        case 'restart':
            exec("kill -9 `ps -ef | grep -E '".$cmd."|vmstat' |grep -v 'grep'| awk '{print $2}'`");
            echo "Kill all process success.\n";
            asyncCaller(syncServer());
            break;
        default:
